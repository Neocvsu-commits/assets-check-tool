...
class ASSETSCHECKNEXT_OT_HeaderTooltip(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        tt_dict = {
            ...
            "命名-规范": "SOP：物体 SM_物体名，材质 MI_材质名，贴图 T_；.001 等重复后缀与非支持字符按黄色提示",
            "名数-匹配": "检测：物体名称是否与其网格数据块（Object Data）名称一致（忽略 .001 等重复后缀）。在 Blender 中复制物体时网格数据名会保留原名，导致物体名和数据名不匹配",
        }
